...
Original file is located at
    https://colab.research.google.com/drive/1erVI91mvPE5eoUSU4yK9biukBlPxy887
...
le = LabelEncoder()
tweets_df['subtask_a'] = le.fit_transform(tweets_df['subtask_a']) 
tweets_df.head()
...
# loc function showing the tweets that contain #MAGA and are non-offensive
tweets_df.loc[(tweets_df['tweet'].str.contains('#MAGA')) & (tweets_df['subtask_a'] == 0)].head()

"""**Visualisation on the Tweets**"""

# defines the method to handle the hashtag data 
def hashtag_extract(x):
  hashtags = []
  for i in x:
    # finds the infomation contained within hashtags
    ht = re.findall(r"#(\w+)", i)
    hashtags.append(ht)

  return hashtags
  
# defines three expressions involving positive, negative and all tweets
HT_regular = hashtag_extract(tweets_df['tweet'][tweets_df['subtask_a'] == 0])
HT_negative = hashtag_extract(tweets_df['tweet'][tweets_df['subtask_a'] == 1])
HT_most = hashtag_extract(tweets_df['tweet'])

# instatiates the set of hashtags
HT_regular = sum(HT_regular, [])
HT_negative = sum(HT_negative, [])
HT_most = sum(HT_most, [])

# finds the hashtags for non-offensive tweets
a = nltk.FreqDist(HT_regular)
d = pd.DataFrame({'Hashtag': list(a.keys()),
                 'Count': list(a.values())})

# sets up the bar chart to display the non-offensive hashtags
d = d.nlargest(columns='Count', n = 10)
plt.figure(figsize=(16,5))
ax = sns.barplot(data=d, x = 'Hashtag', y='Count')
ax.set(ylabel = 'Count')
ax.set_title('Hashtags Associated with Non-Offensive Tweets.')
plt.show()

# finds the hashtags for offensive tweets
b = nltk.FreqDist(HT_negative)
e = pd.DataFrame({'Hashtag': list(b.keys()),
                 'Count': list(b.values())})

# sets up the bar charts for offensive tweets
e = e.nlargest(columns='Count', n = 10)
plt.figure(figsize=(16,5))
ax = sns.barplot(data=e, x = 'Hashtag', y='Count')
ax.set(ylabel = 'Count')
ax.set_title('Hashtags Associated with Offensive Tweets.')
plt.show()

# finds the most common occuring hashtags
w = nltk.FreqDist(HT_most)
t = pd.DataFrame({'Hashtag': list(w.keys()),
                 'Count': list(w.values())})

# sets up the barchart for the common hashtags
t = t.nlargest(columns='Count', n = 10)
plt.figure(figsize=(16,5))
ax = sns.barplot(data=t, x = 'Hashtag', y='Count')
ax.set(ylabel = 'Count')
ax.set_title('The Most Common Hashtags to Appear in the Dataset.')
plt.show()
...
sns.countplot(x = 'subtask_a',  data = barData, palette = 'magma')
plt.title('The Amount of Offensive and Non-Offensive Tweets')
...
xtrain_bow, xvalid_bow, ytrain, yvalid = train_test_split(train_bow, tweets_df['subtask_a'], random_state=42, test_size=0.2)
...
ax.set_yticklabels(['Non-Offensive','Offensive'])
...
# set up a training and testing split 
xtrain_tfidf, xvalid_tfidf, ytrain_tfidf, yvalid_tfidf = train_test_split(train_tfidf, tweets_df['subtask_a'], random_state=42, test_size=0.2)
...
Y = pd.get_dummies(tweets_df['subtask_a']).values
...
model_f1.compile(loss='categorical_crossentropy', optimizer='adam', metrics=[get_f1])
...
"""**LSTM Model Default (Accuracy Score)**"""
...
# find the loss and accuracy of the network
lstm_2 = model_f1.evaluate(X_test,Y_test)
print('Test set\n  Loss: {:0.3f}\n  Accuracy: {:0.3f}'.format(accr[0],accr[1]))
...
tweets_df.drop(['subtask_b', 'subtask_c'], axis = 1, inplace = True, errors = 'ignore')
tweets_df.head()
...
xtrain, ytrain, xvalid, yvalid = train_test_split(tweets_df, tweets_df['subtask_a'], test_size=0.2)
...
predictions, raw_outputs = model.predict(['stupid libtard'])
...
predictions, raw_outputs = model.predict(['happy love you'])

predictions

predictions, raw_outputs = model.predict(['this sentence hates stupid libtards'])
...
data = tweets_df[['tweet', 'subtask_a']].rename(columns={"tweet":"text", "subtask_a":"labels"})
...
text_train, y_train = tweets_df.tweet, tweets_df.subtask_a
...
compare_normalization(u"The liberals are bad and Trump is good." "The conservatives are right about gun control.")
...
stopped_tokens = [i for i in text_train if not i in en_stop]
...
# Create a ClassificationModel
model_albert = ClassificationModel('albert', 'albert-large-v2', num_labels=2, use_cuda=False)
...
# Train the model
model_albert.train_model(xtrain)
...
result, model_outputs, wrong_predictions = model_albert.eval_model(eval_df, f1=f1_multiclass, acc=accuracy_score)
...
# Create a ClassificationModel
model_roberta = ClassificationModel('roberta', 'roberta-large', num_labels=2, use_cuda=False)
...
result, model_outputs, wrong_predictions = model_roberta.eval_model(eval_df, f1=f1_multiclass, acc=accuracy_score)
...
model = AutoModelWithLMHead.from_pretrained("vinai/bertweet-base")

"""**Test Data**"""

# call and display the dataframe
tweets_test = pd.read_csv('drive/My Drive/OLIDv1.0/testset-levela.tsv', delimiter="\t")
tweets_test.head()

tweets_test = tweets_test.set_index('id')

# call and display the dataframe
tweets_labels = pd.read_csv('drive/My Drive/OLIDv1.0/labels-levela.csv', names=['id', 'subtask_a'])
tweets_labels.head()

tweets_labels = tweets_labels.set_index('id')

tweets_df = pd.concat([tweets_test, tweets_labels], axis=1, join='inner')
